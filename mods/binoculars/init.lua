...
local S = engine.get_translator("binoculars")
...
function binoculars.update_player_property(player)
	...
	if player:get_inventory():contains_item(
		...
		new_zoom_fov = 10
	elseif engine.is_creative_enabled(player:get_player_name()) then
		new_zoom_fov = 15
	end
...
local function cyclic_update()
	...
	engine.after(4.7, cyclic_update)
end
...
engine.register_craftitem("binoculars:binoculars", {
