...
local S = engine.get_translator("screwdriver")
...
-- simplified version of engine:builtin/game/falling.lua#L148.
local function check_attached_node(pos, rotation)
	local d = engine.wallmounted_to_dir(rotation)
	local p2 = vector.add(pos, d)
	local n = engine.get_node(p2).name
	local def2 = engine.registered_nodes[n]
...
screwdriver.rotate.wallmounted = function(pos, node, mode)
	...
	if engine.get_item_group(node.name, "attached_node") ~= 0 then
...
screwdriver.handler = function(itemstack, user, pointed_thing, mode, uses)
	...
	local ndef = engine.registered_nodes[node.name]
	if not ndef then
		return itemstack
	end
...
-- Screwdriver
engine.register_tool("screwdriver:screwdriver", {
...
engine.register_alias("screwdriver:screwdriver1", "screwdriver:screwdriver")
engine.register_alias("screwdriver:screwdriver2", "screwdriver:screwdriver")
engine.register_alias("screwdriver:screwdriver3", "screwdriver:screwdriver")
engine.register_alias("screwdriver:screwdriver4", "screwdriver:screwdriver")
