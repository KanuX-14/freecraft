...
local S = engine.get_translator("keys")

--
-- Craftitems
--

engine.register_craftitem("keys:skeleton_key", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local node_reg = engine.registered_nodes[node.name]
		...
		if secret then
			local inv = engine.get_inventory({type="player", name=user:get_player_name()})
			...
			meta:set_string("description", S("Key to @1's @2", user:get_player_name(),
				engine.registered_nodes[node.name].description))

			if itemstack:get_count() == 0 then
				itemstack = new_stack
			else
				if inv:add_item("main", new_stack):get_count() > 0 then
					engine.add_item(user:get_pos(), new_stack)
...
engine.register_craftitem("keys:key", {
	description = S("Key"),
