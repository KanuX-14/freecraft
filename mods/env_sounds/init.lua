...
if engine.settings:get_bool("river_source_sounds") then
...
local function update_sound(player)
  ...
  local pos = engine.find_nodes_in_area(areamin, areamax, cache_triggers, true)
...
local function cyclic_update()
  for _, player in pairs(engine.get_connected_players()) do
    update_sound(player)
  end
  engine.after(3.5, cyclic_update)
end
