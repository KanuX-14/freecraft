...
function creative.init_creative_inventory(player)
	...
	engine.create_detached_inventory("creative_" .. player_name, {
		allow_move = function(inv, from_list, from_index, to_list, to_index, count, player2)
			local name = player2 and player2:get_player_name() or ""
			if not engine.is_creative_enabled(name) or
					to_list == "main" then
				return 0
			end
			return count
		...
		allow_take = function(inv, listname, index, stack, player2)
			local name = player2 and player2:get_player_name() or ""
			if not engine.is_creative_enabled(name) then
				return 0
			end
			return -1
		end,
		...
		on_take = function(inv, listname, index, stack, player2)
			if stack and stack:get_count() > 0 then
				engine.log("action", player_name .. " takes " .. stack:get_name().. " from creative inventory")
			end
		end,
	}, player_name)
...
local function description(def, lang_code)
	...
	if lang_code then
		s = engine.get_translated_string(lang_code, s)
	end
	return s:gsub("\n.*", "") -- First line only
end
...
function creative.update_creative_inventory(player_name, tab_content)
	local inv = player_inventory[player_name] or
			creative.init_creative_inventory(engine.get_player_by_name(player_name))
	local player_inv = engine.get_inventory({type = "detached", name = "creative_" .. player_name})
	...
	local lang
	local player_info = engine.get_player_information(player_name)
...
local trash = engine.create_detached_inventory("trash", {
...
creative.register_tab("all", S("All"), engine.registered_items)
...
function sfinv.get_homepage_name(player)
	if engine.is_creative_enabled(player:get_player_name()) then
		return "creative:all"
	else
