...
if engine.settings:get_bool("enable_weather") == false then
	return
end
...
local function update_clouds()
	...
	nobj_density = nobj_density or engine.get_perlin(np_density)
	nobj_thickness = nobj_thickness or engine.get_perlin(np_thickness)
	nobj_speedx = nobj_speedx or engine.get_perlin(np_speedx)
	nobj_speedz = nobj_speedz or engine.get_perlin(np_speedz)
	...
	for _, player in ipairs(engine.get_connected_players()) do
		...
		local humid = engine.get_humidity(player:get_pos()) or 50
...
local function cyclic_update()
	update_clouds()
	engine.after(CYCLE, cyclic_update)
end


engine.after(0, cyclic_update)
